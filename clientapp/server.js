...
const doCallEndpoint = function () {
  return new Promise((resolve, reject) => {
    return fetchJwtToken()
      .then((jwtToken) => {
        const options = {
          host: "providerapp.enablement.cn40.apps.platform.sapcloud.cn",
          path: "/getData",
          method: "GET",
          headers: {
        ...
        const req = https.request(options, (res) => {
          ...
          if (status !== 200 && status !== 201) {
            return reject(
              new Error(
                `Failed to call endpoint. Error: ${status} - ${res.statusMessage}`
              )
            );
          }
  ...
  return new Promise((resolve, reject) => {
    const options = {
      ...
      headers: {
        Authorization:
          "Basic " +
          Buffer.from(OA_CLIENTID + ":" + OA_SECRET).toString("base64"),
      },
    };
